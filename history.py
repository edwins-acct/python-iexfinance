...
print(f"{'Date':>10}{'Close':>10}{'Change %':>10}{'Volume':>12}{'AvgVol':>10}")
t_price = f"{parsed['quote']['latestPrice']:.2f}".join('$ ')
x = str(parsed['quote']['changePercent'])
if x.startswith("-"): 
    t_chgPct = colored(f"{parsed['quote']['changePercent']:.2f}".join(' %'),'red')
else:
    t_chgPct = colored(f"{parsed['quote']['changePercent']:.2f}".join(' %'),'green')
...
print(f"{t_date:<10}{t_price:>10}{t_chgPct:>20}{t_vol1:>12}{t_chgVol:>10}")
...
#    print(repr(chgPct))  #prints special characters;normally invisible
    ...
    print(f"{date:<10}{close:>10}{chgPct:>20}{vol:>12}{chgVol:>10}")
